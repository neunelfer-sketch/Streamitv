 ...
 *
 * Drei Wege auf denselben Stand, weil die Aufnahmefunktion es kurzzeitig in
 * zwei Fassungen (5 und 6) gab. Wer sie nie hatte, steht auf 4. Alle drei
 * landen auf 7, und keiner verliert dabei etwas: Favoriten und Verlauf
 * bleiben unangetastet, weg fällt nur die Tabelle, die es nicht mehr gibt.
 *
 * Bewusst eine Migration statt eines Verwerfens der Datenbank: Ein
 * Verwerfen nähme dem Zuschauer Favoriten und Verlauf – Daten, die es nur
 * lokal gibt und die kein Neuladen vom Panel wiederbringt.
 */
private val DROP_RECORDINGS = "DROP TABLE IF EXISTS `recordings`"

private val MIGRATION_4_7 = object : Migration(4, 7) {
    override fun migrate(db: SupportSQLiteDatabase) = db.execSQL(DROP_RECORDINGS)
}
...
object AppModule {
    ...
    fun provideOkHttpClient(proxySettings: ProxySettings): OkHttpClient = OkHttpClient.Builder()
        ...
        // Programmzeitschrift, Update-Prüfung, Live-Streams und Filme teilen
        // sich diesen Client (der Player greift über OkHttpDataSource darauf
        // zu, siehe PlayerFactory).
    ...
    fun provideDatabase(@ApplicationContext context: Context): AppDatabase =
        Room.databaseBuilder(context, AppDatabase::class.java, AppDatabase.NAME)
            ...
            // Der Weg auf 7 bekommt ausdrücklich eine echte Migration:
            // Ein Verwerfen nähme dem Zuschauer Favoriten und Verlauf weg –
            // Daten, die es nur hier gibt und die kein Neuladen vom Panel
            // wiederbringt.
            .addMigrations(MIGRATION_4_7, MIGRATION_5_7, MIGRATION_6_7)
            .build()
